...
function App() {
  ...
  const [slicingStatus, setSlicingStatus] = useState({
    active: false,
    percent: 0,
    message: "",
  });
  ...
  const clampToViewport = (pos, size) => {
    ...
    return {
      x: Math.max(
        size.w / 2 + margin,
        Math.min(width - size.w / 2 - margin, pos.x),
      ),
      y: Math.max(
        size.h / 2 + margin + topBarHeight,
        Math.min(height - size.h / 2 - margin, pos.y),
      ),
    };
  };
    ...
    if (
      ...
      const timer = setTimeout(() => {
        ...
        if (micDevices.length > 0) {
          ...
          const queryDevice = micDevices.find(
        ...
        console.log(
          "Auto-connecting to model with device:",
          deviceName,
          "Index:",
          index,
        );
  ...
  useEffect(() => {
    ...
    socket.on("cad_data", (data) => {
      ...
      if (!elementPositions.cad) {
        ...
        const clamped = clampToViewport(
          { x: window.innerWidth / 2 + 150, y: window.innerHeight / 2 },
          size,
        );
        setElementPositions((prev) => ({
          ...prev,
    ...
    socket.on("browser_frame", (data) => {
      ...
      if (!elementPositions.browser) {
        ...
        const clamped = clampToViewport(
          { x: window.innerWidth / 2 - 200, y: window.innerHeight / 2 },
          size,
        );
        setElementPositions((prev) => ({
          ...prev,
        ...
        prev.map((d) => {
          if (d.ip === data.ip) {
            ...
            return {
              ...
              brightness:
                data.brightness !== null ? data.brightness : d.brightness,
            };
          }
          return d;
        }),
    ...
    socket.on("print_status_update", (data) => {
      ...
      if (data.state && data.state.toLowerCase().includes("print")) {
        ...
        data.state &&
        (data.state.toLowerCase() === "idle" ||
          data.state.toLowerCase() === "standby" ||
    ...
    socket.on("initiate_restart", () => {
      console.log(
        "[RESTART] Received restart signal from backend. Relaying to Electron main process.",
      );
    ...
    navigator.mediaDevices.enumerateDevices().then((devs) => {
      ...
      if (
        savedSpeakerId &&
        audioOutputs.some((d) => d.deviceId === savedSpeakerId)
      ) {
      ...
      if (
        savedWebcamId &&
        videoInputs.some((d) => d.deviceId === savedWebcamId)
      ) {
    ...
    const initVision = async () => {
      try {
        ...
        await Promise.all([
          fetch("/hand_landmarker.task").then((r) => {
            if (!r.ok) throw new Error("Hand model missing");
          }),
          fetch("/face_landmarker.task").then((r) => {
            if (!r.ok) throw new Error("Face model missing");
          }),
        ]);
  ...
  const startVideo = async () => {
    try {
      ...
      console.log(
        "Starting video loop with webcam:",
  ...
  const predictWebcam = () => {
      ...
      if (frameCountRef.current % 5 === 0) {
        ...
        if (transCanvas) {
          ...
          transCanvas.toBlob(
            ...
            "image/jpeg",
            0.6,
          ); // Slightly higher compression for speed
        }
      }
    }
    ...
    if (
      ...
      if (frameCountRef.current % 100 === 0) {
        console.log(
          "Tracking loop running... Last result:",
          results.landmarks.length > 0 ? "Hand Found" : "No Hand",
        );
      }
      ...
      if (results.landmarks && results.landmarks.length > 0) {
        ...
        smoothedCursorPosRef.current.x =
          smoothedCursorPosRef.current.x +
          (targetX - smoothedCursorPosRef.current.x) * lerpFactor;
        smoothedCursorPosRef.current.y =
          smoothedCursorPosRef.current.y +
          (targetY - smoothedCursorPosRef.current.y) * lerpFactor;
        ...
        if (snapStateRef.current.isSnapped) {
          ...
          if (
            nowTime - lastInteractiveSearchTimeRef.current > 1000 ||
            interactiveTargetsCacheRef.current.length === 0
          ) {
            interactiveTargetsCacheRef.current = Array.from(
        ...
        const isFist =
          isFingerFolded(8, 5) &&
          isFingerFolded(12, 9) &&
          isFingerFolded(16, 13) &&
          isFingerFolded(20, 17);
        ...
        const wristNormX = Math.max(
          0,
          Math.min(1, (wristRawX - 0.5) * SENSITIVITY + 0.5),
        );
        const wristNormY = Math.max(
          0,
          Math.min(1, (wrist.y - 0.5) * SENSITIVITY + 0.5),
        );
        ...
        if (isFist) {
          ...
          if (activeDragElementRef.current) {
            ...
            if (Math.abs(dx) > 0.5 || Math.abs(dy) > 0.5) {
              const currentPos =
                elementPositionsRef.current[activeDragElementRef.current];
              if (currentPos) {
  ...
  const updateElementPosition = (id, x, y) => {
    setElementPositions((prev) => {
      ...
      if (id === "chat") {
        ...
        newX = Math.max(
          size.w / 2 + margin,
          Math.min(width - size.w / 2 - margin, newX),
        );
        ...
        newY = Math.max(
          size.h / 2 + margin,
          Math.min(height - size.h / 2 - margin, newY),
        );
      }
    ...
    <div className="h-screen w-screen bg-gray2 text-gray11 font-sans overflow-hidden flex flex-col relative selection:bg-gold-900/50 selection:text-white">
      ...
      >
        <div className="flex items-center gap-4 pl-2">
          <h1 className="text-xl font-bold tracking-[0.2em] text-gold9 drop-shadow-[0_0_10px_rgba(255,215,0,0.5)]">
            A.D.A
          </h1>
          <div className="text-[10px] text-gold9/70 border border-gold9/20 px-1 rounded">
            V2.0.0
          </div>
          {/* FPS Counter */}
          {isVideoOn && (
          ...
          {kasaDevices.length > 0 && (
            <div className="flex items-center gap-1.5 text-[10px] text-yellow-400 border border-yellow-500/30 bg-yellow-500/10 px-2 py-0.5 rounded ml-2">
              <span>💡</span>
              <span>
                {kasaDevices.length} Device{kasaDevices.length !== 1 ? "s" : ""}
              </span>
            </div>
          )}
        </div>
        ...
        >
          ...
          <button
            ...
            className="p-1 hover:bg-red-900/50 rounded text-red9 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
      ...
      <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
        ...
        >
          ...
          {isModularMode && (
            <div
              className={`absolute top-2 right-2 text-xs font-bold tracking-widest z-20 ${activeDragElement === "visualizer" ? "text-green-500" : "text-yellow-500/50"}`}
            >
              VISUALIZER
            </div>
          )}
        ...
        {showCadWindow && (
          ...
          >
            ...
            <div
              data-drag-handle
              className="h-8 bg-gray-900/80 border-b border-gold9/20 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing shrink-0"
            >
              <span className="text-xs font-bold tracking-widest text-gold9/70">
                CAD PROTOTYPE
              </span>
              <button
                onClick={() => setShowCadWindow(false)}
                className="text-gray-400 hover:text-red9 hover:bg-red9/20 p-1 rounded transition-colors"
              >
                ✕
              </button>
            </div>
        ...
        <div className="z-20 flex justify-center pb-10 pointer-events-none">
          <ToolsModule
            ...
            onToggleFleetSettings={() =>
              setShowFleetSettings(!showFleetSettings)
            }
